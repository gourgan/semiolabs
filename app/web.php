...
///search with tags from home 
$app->match('/search/{item}', function (Request $request,$item) use ($app) {
			$app['session']->setFlash('success', null);
			$app['session']->setFlash('error', null);           
...
->bind('rss');
 ...
 $app->match('/mostview', function (Request $request) use ($app){
    $app['session']->setFlash('success', null);
			$app['session']->setFlash('error', null);           
                    ...
                    try {
                        ...
                        return $app['twig']->render('/../views/template/mostview.twig', array(
                                    'page' => 'mostview',
                                    'articles' => $articles
                                ));
                    } catch (Exception $e) {

...
/**
 * Base class that represents a row from the 'auteur' table.
 *
 *
 *
...
abstract class BaseAuteur extends BaseObject implements Persistent
{
     ...
     * @return Auteur The current object (for fluent API support)
     */
    ...
    public function setLogin($v)
    {
        ...
        if ($this->login !== $v) {
            $this->login = $v;
            $this->modifiedColumns[] = AuteurPeer::LOGIN;
        }
    ...
    public function setPassword($v)
    {
        ...
        if ($this->password !== $v) {
            $this->password = $v;
            $this->modifiedColumns[] = AuteurPeer::PASSWORD;
        }
    ...
    public function setDescription($v)
    {
        ...
        if ($this->description !== $v) {
            $this->description = $v;
            $this->modifiedColumns[] = AuteurPeer::DESCRIPTION;
        }
    ...
    public function setImage($v)
    {
        ...
        if ($this->image !== $v) {
            $this->image = $v;
            $this->modifiedColumns[] = AuteurPeer::IMAGE;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            return $startcol + 5; // 5 = AuteurPeer::NUM_HYDRATE_COLUMNS.
    ...
    public function reload($deep = false, PropelPDO $con = null)
    {
        ...
        $stmt = AuteurPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    public function delete(PropelPDO $con = null)
    {
        ...
        try {
            $deleteQuery = AuteurQuery::create()
    ...
    public function save(PropelPDO $con = null)
    {
        ...
        try {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                AuteurPeer::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
            $con->commit();
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(AuteurPeer::ID)) {
            $modifiedColumns[':p' . $index++]  = '`id`';
        }
        if ($this->isColumnModified(AuteurPeer::LOGIN)) {
            $modifiedColumns[':p' . $index++]  = '`login`';
        }
        if ($this->isColumnModified(AuteurPeer::PASSWORD)) {
            $modifiedColumns[':p' . $index++]  = '`password`';
        }
        if ($this->isColumnModified(AuteurPeer::DESCRIPTION)) {
            $modifiedColumns[':p' . $index++]  = '`description`';
        }
        if ($this->isColumnModified(AuteurPeer::IMAGE)) {
            $modifiedColumns[':p' . $index++]  = '`image`';
        }

        $sql = sprintf(
            'INSERT INTO `auteur` (%s) VALUES (%s)',
    ...
    {
        if (isset($alreadyDumpedObjects['Auteur'][$this->getPrimaryKey()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['Auteur'][$this->getPrimaryKey()] = true;
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(AuteurPeer::LOGIN)) $criteria->add(AuteurPeer::LOGIN, $this->login);
        if ($this->isColumnModified(AuteurPeer::PASSWORD)) $criteria->add(AuteurPeer::PASSWORD, $this->password);
        if ($this->isColumnModified(AuteurPeer::DESCRIPTION)) $criteria->add(AuteurPeer::DESCRIPTION, $this->description);
        if ($this->isColumnModified(AuteurPeer::IMAGE)) $criteria->add(AuteurPeer::IMAGE, $this->image);
     ...
     * objects.
     *
     * @param object $copyObj An object of Auteur (or compatible) type.
     ...
     *
     * @param boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return Auteur Clone of current object.
     * @throws PropelException
     */
     ...
     * If this Auteur is new, it will return
     ...
     * Otherwise if this Auteur is new, it will return
     * an empty collection; or if this Auteur has previously
     * been saved, it will retrieve related Articles from storage.
     *
     * This method is protected by default in order to keep the public
     * api reasonable.  You can provide public methods for those you
     * actually need in Auteur.
     *
     * @param Criteria $criteria optional Criteria object to narrow the query
     * @param PropelPDO $con optional connection object
     ...
     * @return PropelObjectCollection|Article[] List of Article objects
     */
    public function getArticlesJoinType($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
    {
        $query = ArticleQuery::create(null, $criteria);
    ...
    {
        return (string) $this->exportTo(AuteurPeer::DEFAULT_STRING_FORMAT);
    }
